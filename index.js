...
async function run() {
  try {
    ...
    // get one image post
    app.get('/post/image/:id', async (req, res) => {
      const id = req.params.id
      const query = { _id: ObjectId(id) }
      const result = await imagePostCollection.findOne(query)
      res.send(result)
    })

    // imagePost Delete
    app.delete('/post/image/:id', async (req, res) => {
      const id = req.params.id
      const query = { _id: ObjectId(id) }
      const result = await imagePostCollection.deleteOne(query)
      res.send(result)
    })
  } finally {
  }
